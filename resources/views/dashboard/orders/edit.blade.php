...
<div class="max-w-3xl mx-auto bg-white shadow rounded-lg p-8">
    ...
    <form action="{{ route('orders.update', $order) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <div id="order-items">
                ...
                <div class="flex space-x-2 mb-2 order-item-row">
                    ...
                    <input type="number" name="quantities[]" min="1" value="{{ $item->quantity }}" class="block w-1/4 rounded-md border-gray-300" placeholder="Qty">
        ...
        <div class="mb-4">
            ...
            <textarea name="notes" id="notes" rows="3" class="mt-1 block w-full rounded-md border-gray-300">{{ old('notes', $order->notes) }}</textarea>
            @error('notes')
                <span class="text-red-500 text-xs">{{ $message }}</span>
            @enderror
        </div>
        <div class="flex justify-end">
            <button type="submit" class="bg-primary-600 hover:bg-primary-700 text-white px-6 py-2 rounded-lg font-semibold">Update Order</button>
        </div>
    </form>
</div>
<script>
